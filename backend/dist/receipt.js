...
const titleCase = (s) => s.toLowerCase().replace(/(^|\s)\S/g, (c) => c.toUpperCase()).replace(/^./, (c) => c.toUpperCase());
...
export function parseReceiptLine(input) {
    ...
    if (!cleaned || cleaned.replace(/ /g, '').length < 3)
        return null;
