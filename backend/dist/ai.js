...
const has = (name, words) => {
    const n = name.toLowerCase();
    return words.some((w) => n.includes(w));
};
export function activeFilters(diet) {
    if (!diet)
        return [];
    const f = [];
    if (diet.vegetarian)
    ...
    if (diet.lowCarb)
        f.push('Low-carb');
    for (const a of diet.allergies)
        f.push(`No ${a.toLowerCase()}`);
    return f;
}
function exclusionReason(name, diet) {
    if (!diet)
        return null;
    for (const a of diet.allergies)
        if (has(name, ALLERGEN_WORDS[a] ?? [a.toLowerCase()]))
            return `${a} allergy`;
    if (diet.vegetarian && has(name, MEAT_WORDS))
        return 'vegetarian';
    if (diet.glutenFree && has(name, GLUTEN_WORDS))
        return 'gluten-free';
    if (diet.lowCarb && has(name, CARB_WORDS))
        return 'low-carb';
    return null;
}
// ── Portions ───────────────────────────────────────────────────────────────
...
export function portion(item) {
    ...
    const cap = { g: 250, kg: 0.3, ml: 200, l: 0.3, pieces: 2, pcs: 2, '': 2 };
    if (has(item.name, ['oil']) && (u === 'ml' || u === 'l'))
        return round(Math.min(item.qty, u === 'l' ? 0.03 : 30));
    if (u in cap)
        return round(Math.min(item.qty, cap[u]));
    if (has(item.name, ['spinach', 'kale', 'lettuce']))
        return round(Math.min(item.qty, 1));
 ...
 */
export async function generateRecipe(picked, diet, variant, opts = {}) {
    const started = Date.now();
    await new Promise((r) => setTimeout(r, 700 + Math.random() * 600));
    if (opts.simulateFailure)
        throw new AiUnavailableError('AI service unavailable');
    const excluded = [];
    const usable = [];
    for (const p of picked) {
        const why = exclusionReason(p.name, diet);
        if (why)
            excluded.push({ name: p.name, reason: why });
        else
            usable.push(p);
    }
    if (!usable.length) {
...
function score(name) {
    if (has(name, MEAT_WORDS) || has(name, ['egg', 'tofu']))
        return 3;
    if (has(name, ['spinach', 'tomato', 'carrot', 'onion', 'kale', 'pepper', 'mushroom', 'banana']))
        return 2;
    if (has(name, ['rice', 'oats', 'pasta', 'noodle']))
        return 1;
    return 0;
}
