...
import type { DietProfile } from '../types.js';
...
export function getDiet(userId: string): DietProfile {
  return (
    db.diets.find((d) => d.userId === userId) ?? {
      userId,
      vegetarian: false,
      glutenFree: false,
      lowCarb: false,
...
profileRouter.put('/diet', (req, res) => {
  ...
  const allergies: string[] = Array.isArray(body.allergies)
    ? body.allergies.filter((a: unknown): a is string => typeof a === 'string' && ALLERGIES.includes(a))
    : [];
  ...
  for (const [key, label] of FILTERS) {
    if (next[key] !== prev[key]) addRecord(userId, 'diet_filter', next[key] ? 'granted' : 'withdrawn', label);
  }
  for (const a of ALLERGIES) {
    const was = prev.allergies.includes(a);
    const is = next.allergies.includes(a);
    if (was !== is) addRecord(userId, 'diet_filter', is ? 'granted' : 'withdrawn', `Allergy: ${a}`);
  }
  const anyOn = next.vegetarian || next.glutenFree || next.lowCarb || next.allergies.length > 0;
  const { dietConsent } = consentState(userId);
  if (anyOn && !dietConsent) addRecord(userId, 'diet_consent', 'granted', 'Use my dietary filters');
...
profileRouter.post('/consents/:kind', (req, res) => {
  ...
  if (kind !== 'ai' && kind !== 'diet') throw new HttpError(404, 'unknown_consent', 'Unknown consent type.');
  ...
  if (!!granted !== current) {
    addRecord(
      ...
      kind === 'ai' ? 'Pantry data sent to AI service' : 'Use my dietary filters',
    );
  }
